...
export function UserForm({userData, onSubmitFunction, login, updating, userPhotoUrl}){
    ...
    const PasswordField = ()=>{
        /**
         * En este punto, no vale la pena modular el PasswordField como un componente independiente
         * ya que, nunca se volvera a usar 
         */
        return (
            <FormField  label="Contraseña" errors={errors.password && errors.password.message}>
                <input className="password-input"type="password"name="password"id="password"{...register("password", BASE_PASSWORD_CONSTRAINTS)}/>
            </FormField>
        )
    }
    const UsernameField = ()=>{
        return (
            <FormField  label="Usuario" errors={errors.username && errors.username.message}>
    ...
    return (
        <div className="user-form-container">
            <Form onSubmitFunction={onSubmit} buttonMsg={updating ? "Actualizar" : (login ?  "Acceder" : "Registrar")}> 
            {
                login ?
                    <>
                        {UsernameField()}
            ...
            {!login && <UserPhoto withInput photoFileSetter={setCurrentPhotoFile} url={updating ?  userPhotoUrl : null}/>}
        </div>
    )
}