...
def set_photo_link(sended_data, view_type, current_photo_link=None):
    """
    ...
    if 'photo' not in sended_data :
        ...
        sended_data['photo_link'] =  save_image_cloudinary(sended_data['photo']) if view_type == "creating" else (overwrite_image_cloudinary(sended_data['photo']) if current_photo_link is not None else save_image_cloudinary(sended_data['photo']))
